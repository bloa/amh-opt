...
namespace opt {
  class fitness_max : public fitness_base<double,double> {
  public:

    bool operator==(const fitness_max& _f) const {
    ...
    int scalarComp(fitness_max& _f) const {
    ...
    bool lexicoComp(const fitness_max& _f) const {
      int s = objectives_.size();
      for (int k=0; k<s; k++)
